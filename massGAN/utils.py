...
class Utils:
    ...
    @staticmethod
    def get_rotated_mesh(mesh: trimesh.Trimesh, angle: float, axis: Tuple[int] = Config.Z) -> trimesh.Trimesh:
    ...
    def plot_interpolated_masses(
        ...
        with torch.no_grad():
            ...
            for nmi in range(num_masses - 1):
                ...
                interpolated_noises = [
                    curr_noise + (j / (num_steps - 1)) * (next_noise - curr_noise) for j in range(num_steps)
                ]
...
class Preprocessor(Utils):
    ...
    def preprocess(self) -> None:
        ...
        for data_name in os.listdir(Config.DATA_ORIGINAL_DIR_MERGED):
            ...
            each_obj_data_path = os.path.join(Config.DATA_ORIGINAL_DIR_MERGED, data_name, data_name + Config.OBJ_FORMAT)
            ...
            for each_mesh_to_preprocess in mesh_to_preprocess:
                ...
                if self.use_to_rotate:
                    ...
                    while True:
                        ...
                        if rotation_degree >= Config.ROTATION_MAX or np.isclose(rotation_degree, Config.ROTATION_MAX):
                            break
