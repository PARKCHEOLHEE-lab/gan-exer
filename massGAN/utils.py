...
class Utils:
    ...
    @staticmethod
    def plot_interpolated_masses(generator, num_steps=10, num_masses=2):
        """
        Plot the interpolation between multiple predicted voxel-shaped masses generated by a GAN model.

        Args:
            generator (nn.Module): The GAN generator model.
        ...
        # Generate random noise vectors for the specified number of masses
        noises = [torch.randn(1, Config.Z_DIM, 1, 1, 1).to(Config.DEVICE) for _ in range(num_masses)]

        # Create a list to store all intermediate masses
        interpolated_masses = []

        # Interpolate between each pair of adjacent masses
        for i in range(num_masses - 1):
            noise1, noise2 = noises[i], noises[i + 1]

            # Create intermediate noise vectors through linear interpolation
            interpolated_noises = [noise1 + (j / (num_steps - 1)) * (noise2 - noise1) for j in range(num_steps)]

            # Generate intermediate voxel grids
            interpolated_masses.extend(
                generator(noise).squeeze().cpu().detach().numpy() 
                for noise in interpolated_noises
            )

        # Plot the interpolated masses using the plot_binvox function
        Utils.plot_binvox(data_list=interpolated_masses, plot_voxels=False)
